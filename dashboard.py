import streamlit as st
import pandas as pd
import gspread

# --- HARD-CODED AUTHENTICATION (RELIABLE FIX) ---
# This dictionary contains your credentials exactly as required
CREDS_DICT = {
    "type": "service_account",
    "project_id": "sabin-erp-engine",
    "private_key_id": "0990e08a6dbe187eb04ecafdf7700e18cbcc0603",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----",
    "client_email": "sabin-bot@sabin-erp-engine.iam.gserviceaccount.com",
    "client_id": "102532041684182521279",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/sabin-bot%40sabin-erp-engine.iam.gserviceaccount.com",
    "universe_domain": "googleapis.com"
}

# --- CONNECT AND DISPLAY DATA ---
st.title("Sabin ERP Dashboard")

try:
    # Authenticate directly using the dictionary above
    gc = gspread.service_account_from_dict(CREDS_DICT)
    
    # Open the spreadsheet
    sheet_url = "https://docs.google.com/spreadsheets/d/1nfD90CqHeb0TFSbivR9_rvoBlECJ37FgA5j-2G2Eyzc/edit"
    sh = gc.open_by_url(sheet_url)
    
    # Get the data
    data = sh.get_worksheet(0).get_all_records()
    df = pd.DataFrame(data)
    
    st.success("Connection Successful!")
    st.dataframe(df)

except Exception as e:
    st.error(f"Connection error: {e}")