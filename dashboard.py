import streamlit as st
import pandas as pd
import gspread
import altair as alt
import io
import os
from datetime import datetime

# --- PERMANENT CREDENTIALS (NO SECRETS BOX NEEDED) ---
CREDS_DICT = {
  "type": "service_account",
  "project_id": "sabin-erp-engine",
  "private_key_id": "0990e08a6dbe187eb04ecafdf7700e18cbcc0603",
  "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----",
  "client_email": "sabin-bot@sabin-erp-engine.iam.gserviceaccount.com",
  "client_id": "102532041684182521279",
  "auth_uri": "https://accounts.google.com/o/oauth2/auth",
  "token_uri": "https://oauth2.googleapis.com/token",
  "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
  "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/sabin-bot%40sabin-erp-engine.iam.gserviceaccount.com",
  "universe_domain": "googleapis.com"
}
GSHEET_URL = "https://docs.google.com/spreadsheets/d/1nfD90CqHeb0TFSbivR9_rvoBlECJ37FgA5j-2G2Eyzc/edit?gid=0#gid=0"

# --- CORE LOGIC ---
@st.cache_resource
def get_gc():
    return gspread.service_account_from_dict(CREDS_DICT)

def load_inventory():
    try:
        sh = get_gc().open_by_url(GSHEET_URL)
        return pd.DataFrame(sh.get_worksheet(0).get_all_records())
    except:
        return pd.DataFrame()

# --- APP LAYOUT ---
st.set_page_config(page_title="SABIN ERP", layout="wide")
st.title("SABIN ERP // Dashboard")

df = load_inventory()

if not df.empty:
    # Sidebar Filters
    status_filter = st.sidebar.multiselect("Status", df["Status"].unique())
    
    # Apply Filters
    if status_filter:
        df = df[df["Status"].isin(status_filter)]
    
    # Display Stats
    st.metric("Total Active Records", len(df))
    st.dataframe(df, use_container_width=True)
else:
    st.warning("No data found. Please check connection.")